// 네이버 금융에서 한국 상장 ETF의 보수·순자산·배당수익률을 가져온다.
// API 라우트(/api/stock/naver)와 서버 컴포넌트가 함께 쓴다.
import { Redis } from "@upstash/redis"

// 환경변수가 없으면 비활성화 - 로컬 개발 시 캐시 없이 동작
const redis =
...
export async function getNaverEtfInfo(symbol: string): Promise<NaverEtfInfo> {
  ...
  if (redis) {
    try {
      const cached = await redis.get<NaverEtfInfo>(cacheKey)
      if (cached) {
        return cached
      }
    } catch {
